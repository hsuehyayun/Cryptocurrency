...
const ASSETS = {
    "USDC": { "mint": "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v", "decimals": 6, "allocation": new Decimal('0.00') },
    "JTO": { "mint": "jtojtomepa8beP8AuQc6eXt5FriJwfFMwQx2v2f9mCL", "decimals": 9, "allocation": new Decimal('0.04') },
    "WIF": { "mint": "EKpQGSJtjMFqKZ9KQanSqYXRcF8fBopzLHYxdM65zcjm", "decimals": 6, "allocation": new Decimal('0.0') },
    "SOL": { "mint": "So11111111111111111111111111111111111111112", "decimals": 9, "allocation": new Decimal('0.960') },
    "JUP": { "mint": "JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN", "decimals": 6, "allocation": new Decimal('0.0') },
    "JLP": {"mint": "27G8MtK7VtTcCHkpASjSDdkWWYfoqT6ggEuKidVJidD4", "decimals": 6, "allocation": new Decimal('0.0') },
    "SHDW": {"mint": "SHDWyBxihqiCj6YekG2GUr7wqKLeLAMK1gHZck9pL6y", "decimals": 9, "allocation": new Decimal('0.0') },
};
...
const RPC_ENDPOINT = "https://delicate-side-reel.solana-mainnet.quiknode.pro/APIKEY/";
const KEYPAIR_PATH = "/Users/hogyzen12/.config/solana/muMuRjyK84ASfKpDTi23es3XZ5vtHScFnN5vvi1K8hy.json";
const REBALANCE_THRESHOLD = new Decimal('0.0042');
const CHECK_INTERVAL = 60;

const STASH_THRESHOLD = new Decimal('42'); // $10 threshold for stashing
...
async function getWalletBalances(walletAddress) {
    ...
    try {
        ...
        if (solBalanceResponse.data.result?.value !== undefined) {
            const solLamports = new Decimal(solBalanceResponse.data.result.value.toString());
        ...
        for (const [asset, details] of Object.entries(ASSETS)) {
            ...
            if (tokenResponse.data.result?.value) {
                ...
                const totalBalance = tokenResponse.data.result.value.reduce((sum, account) => {
                ...
                balances[asset] = totalBalance.div(new Decimal(10).pow(details.decimals));
